10. The compressor must have 4 bands. \\
...
Only a lowpass filter should be designed because of spectral inversion, as described in \autoref{ch:designRealisation}, so instead of a bandpass requirements only one side of the requirements are showed from now on. %From system overview the octave bands are given for frequencies below 530 Hz, see \autoref{tb:freqBands} which meets requirement 8 and 10.
