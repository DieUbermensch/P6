...
\node [Twolineblock, fill=white] (Decisionblock) at ($(Equalizer)+(0,-2)$) {Spectral Analysis};

\node [block, fill=white] (Delay) at ($(Equalizer)+(3.25,0)$) {Delay};

\node [block, fill=gray,text=white] (Limiter) at ($(Delay)+(6.5,0)$) {Limiter};

\node [Twolineblock, fill=white] (AmplitudeDetect) at ($(Delay)+(0,-2)$) {Amplitude Detection};

\node [Twolineblock, fill=white] (NewEQ) at ($(Delay)+(3.25,0)$) {Auto Equalizer};
