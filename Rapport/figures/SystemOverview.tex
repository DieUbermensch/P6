...
%% DSP %%
\node [block, fill=white] (Equalizer) at ($(AudioSource)+(3.5,0)$) {Equalizer};
\node [block, fill=gray,text=white] (Limiter) at ($(Equalizer)+(3.5,0)$) {Limiter};
\node [Twolineblock, fill=white] (Decisionblock) at ($(Equalizer)+(0,-2)$) {Decision block};
...
%% Store Blokke %%
\begin{pgfonlayer}{bg}
\draw[thick, fill=black!20] ($(-1.50,1)+(Equalizer)$) -- ($(1.750,1)+(Limiter)$) -- ($(1.750,-1.25)+(SpectralAnalysis)$) -- ($(-1.50,-1.25)+(Decisionblock)$) -- ($(-1.50,1)+(Equalizer)$);
\node (DSPtag) at ($(1,-1)+(SpectralAnalysis)$) {\textbf{DSP}};

\draw[thick, fill=black!20] ($(-1.50,1)+(Amplifier)$) -- ($(1.750,1)+(Driver)$) -- ($(1.750,-1.25)+(EnclosureDriver)$) -- ($(-1.50,-1.25)+(SensorDriver)$) -- ($(-1.50,1)+(Amplifier)$);
\node (DSPtag) at ($(-.25,-1)+(EnclosureDriver)$) {\textbf{Speaker Enclosure}};
\end{pgfonlayer}

%% Forbindelse %%
\draw[->] (AudioSource) -- (Equalizer);
\draw[->] (Equalizer) -- (Limiter);
