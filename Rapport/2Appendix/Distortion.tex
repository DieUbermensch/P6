...
\end{itemize}
These are classified by looking at the fundamental frequency and is then categorized by how far away, indexed by whole integers, they are from the fundamental frequency. Looking at an ordinary tone, such as $A_4$ which is a tone at 440 Hz the concept is easier to describe. This tone is usually used to tune audio equipment such as guitars and alike. Conforming \autoref{fig:ODDevenTHD} a spectral analysis of $A_4$ tone is shown. \autoref{fig:EvenTHD} shows the 440 Hz tone with even harmonics and \autoref{fig:OddTHD} with odd harmonics. 
...
Generally speaking it is desirable to have even harmonics since they are perceived as "warm and full" sound. Odd harmonics are perceived as "cold and metallic". The reason why it usually is most desirable, is due to the fact that all even harmonics represent the same tone but at one octave higher in frequency. This can be seen in \autoref{tab:NoteA4}, where the tones $A_5$, $A_6$, $A_7$ corresponds to the second, fourth and eight harmonic.
...
\va{$A_n$}{is the amplification of the nth harmonic distortion}{-}\\
\va{$A_1$}{is the amplification of the fundamental frequency}{-}\\
\end{where}
